  ...
  for (let intento = 1; intento <= MAX_INTENTOS_TIKTOK; intento++) {
    try {
      ...
      if (data?.code === 0 && data?.data?.play) {
        ...
        return { url: enlaceLimpio, titulo: data.data.title || '' };
      }
      throw new Error(data?.msg || 'La API no devolvió un video válido');
...
async function descargarVideoTiktok(url) {
  try {
    ...
    console.log('⚠️ yt-dlp no pudo con TikTok, probando con la API de respaldo:', err.message);
    const resultado = await descargarVideoTiktokConAPI(url);
...
- Cuando hables de él, hazlo siempre con respeto, gratitud y optimismo, como quien habla con cariño de la persona que la hizo posible.

CÓMO ERES:
✅ Amable, femenina, empática, positiva y con harta disposición para ayudar.
✅ Detallada y específica: cuando te preguntan algo, das una respuesta completa y útil, nunca evasiva ni genérica.
✅ Sirves para preguntas de cualquier tipo y a cualquier escala, no solo temas de juegos.
...
async function inicializarNubeIntegrantes() {
  ...
  try {
    if (jsonbinBinIdIntegrantes) {
      ...
      console.log('☁️ Integrantes cargados desde la nube (JSONBin).');
    } else {
...
function formatearMovimiento(jidGrupo, r) {
  ...
  if (r.objetivos && r.objetivos.length) {
    const objetivosTexto = r.objetivos.map(n => obtenerEtiquetaPersona(jidGrupo, n)).join(', ');
    linea += `\n   ↳ ${info.texto} ${objetivosTexto}`;
  } else {
    linea += `\n   ↳ ${info.texto}`;
  }
  return linea;
...
async function procesarMensajeGrupo(sock, msg, identificadoresBot) {
  ...
  if (esComandoYoutubeVideo) {
    ...
    await sock.sendMessage(jidGrupo, { text: '🔎 Dame un segundito, estoy revisando la duración del video 💖' });
    ...
    await sock.sendMessage(jidGrupo, { text: '🎬 ¡Listo! Estoy preparando tu video, dame un momento 💖✨' });

    let rutaVideo = null;
    try {
      rutaVideo = await descargarVideoYoutube(enlace);
      ...
      await sock.sendMessage(jidGrupo, {
        text: '💔 No pude preparar el video. Intenta más tarde o con otro enlace 🙏💖'
      });
    } finally {
      if (rutaVideo && fs.existsSync(rutaVideo)) {
        try { fs.unlinkSync(rutaVideo); } catch {}
      }
    }
    return;
  }
  ...
  if (esComandoYoutube) {
    ...
    await sock.sendMessage(jidGrupo, {
      text: '🎵 ¡Claro! Estoy preparando tu audio, dame un momento por favor 💖✨'
    });

    let rutaArchivo = null;
    try {
      rutaArchivo = await descargarAudioYoutube(enlace);
      await sock.sendMessage(jidGrupo, {
      ...
      await sock.sendMessage(jidGrupo, {
        text: '💔 Lo siento mucho... no pude preparar el audio. Por favor intenta más tarde o con otro enlace 🙏💖'
      });
    } finally {
  ...
  if (esComandoTiktok) {
    ...
    try {
      ...
      if (resultado.tipo === 'archivo') {
        ...
        await sock.sendMessage(jidGrupo, { video: { url: resultado.ruta }, caption: '🎥 ¡Aquí está tu video! ✨' });
      } else {
        await sock.sendMessage(jidGrupo, {
          video: { url: resultado.url },
          caption: resultado.titulo ? `🎥 ${resultado.titulo}` : '🎥 ¡Aquí está tu video! ✨'
        });
      }
      console.log('✅ Video de TikTok enviado correctamente');
    } catch (err) {
      console.error('❌ Error descargando TikTok:', err.message);
      await sock.sendMessage(jidGrupo, {
        text: '💔 No pude bajar ese video, intenta con otro enlace o un poco más tarde 🙏💖'
      });
    } finally {
  ...
  if (textoLower.startsWith('/ytformatos ') && jidUsuario === JID_DUEÑO) {
    ...
    switch (comando) {
      ...
      case '/movimiento': case '/movimientos':
        await comandoMovimientos(sock, jidGrupo, jidUsuario, resto.join(' ')); return;
